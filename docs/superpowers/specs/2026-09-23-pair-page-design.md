...
Picking a food fetches `POST /api/recommendations` with that food's vector (`limit`/`offset` only — no `type`/`country`/price filters). While a fetch is in flight, the picker stays visible (all tiles disabled via `FoodPicker`'s `loading` prop) with a "Finding wines for {Food}…" status line above the grid — unlike Discover, Pair has no `localStorage`-restore path that would ever put it in results mode before a fetch completes, so a results-mode skeleton is never reachable and isn't used. A request failure shows `ErrorMessage` with retry. Results paginate via the same "Load more" pattern already used on Explore and Discover, including the stale-request-safe reset (`loadingMore`/`loadMoreError` reset alongside `items` when the food selection changes), matching the fix already applied to Discover's request-guard pattern.
