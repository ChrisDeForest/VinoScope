...
## Known Limitations (found 2026-09-23, during the Pair page's final review)

**Wines with no recorded characteristics score a perfect (100%) match on every request.** In `_distance` (`app/services/recommendations.py`), a wine is skipped per-dimension whenever either the user or the wine has no value there; if a wine has all five characteristic columns `NULL`, `total_weight` stays `0` and the function returns `0.0`, which `_match_score` converts to `1.0` — a perfect match score, sorted to the top of every result set regardless of what was asked for. This was tolerable for Discover, where a sparse user profile flattens scores across the board and dilutes the effect. It's much more exposed by the Pair page (`docs/superpowers/specs/2026-09-23-pair-page-design.md`), which always sends all five dimensions for every food — so the catalog's most under-populated rows become the top "match" for Steak, Dessert, and everything else. Not fixed as part of the Pair page branch (backend changes were explicitly out of scope for that feature). Before prioritizing a fix, check how many rows in the live catalog actually have all-`NULL` characteristics — if none do, this is theoretical. If a fix is needed: give an unscoreable wine (zero overlapping dimensions with the request) the worst distance instead of the best, or exclude it from scored results entirely, rather than treating "nothing to compare" as "perfect match."

**Multi-level preferences (`list[int]`) produce awkward profile-chip phrasing.** `build_profile`/`build_explanation` join multiple levels with `" or "` (e.g. `sweetness: [3, 4]` → `"Off-dry or Sweet"`). This reads fine for Discover's typical 1-2 adjacent levels, but the Pair page's food vectors can span 3 levels (e.g. Cheese's body `[3, 4, 5]` → `"Medium-bodied or Full-bodied or Very full-bodied"`), which is verbose. Cosmetic only, not fixed. If it needs improving, collapse a contiguous run to `"Medium-bodied to very full-bodied"` instead of joining every level with "or".
